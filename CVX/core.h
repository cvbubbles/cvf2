...
template<typename _Tp>
inline void m34ToRt(const Matx<_Tp, 3, 4> &m, Matx<_Tp, 3, 3> &R, Vec<_Tp, 3> &t)
{
	R = Matx<_Tp, 3, 3>(m(0, 0), m(0, 1), m(0, 2), m(1, 0), m(1, 1), m(1, 2), m(2, 0), m(2, 1), m(2, 2));
	t = Vec<_Tp, 3>(m(0, 3), m(1, 3), m(2, 3));
}
template<typename _Tp>
inline Matx<_Tp, 3, 4> m34FromRt(const Matx<_Tp, 3, 3> &R, const Vec<_Tp, 3> &t)
{
	return Matx<_Tp, 3, 4>(R(0, 0), R(0, 1), R(0, 2), t[0], R(1, 0), R(1, 1), R(1, 2), t[1], R(2, 0), R(2, 1), R(2, 2), t[2]);
}
template<typename _Tp>
inline Matx<_Tp,4,4> m34ToM44(const Matx<_Tp, 3, 4> &m)
{
	return Matx<_Tp, 4, 4>(m(0, 0), m(0, 1), m(0, 2), m(0,3), m(1, 0), m(1, 1), m(1, 2), m(1,3), m(2, 0), m(2, 1), m(2, 2), m(2,3), 0, 0, 0, 1);
}
template<typename _Tp>
inline Matx<_Tp, 3, 4> m34FromM44(const Matx<_Tp, 4, 4>& m)
{
	return Matx<_Tp, 3, 4>(m(0, 0), m(0, 1), m(0, 2), m(0, 3), m(1, 0), m(1, 1), m(1, 2), m(1, 3), m(2, 0), m(2, 1), m(2, 2), m(2, 3));
}

template<typename _Tp>
inline Matx<_Tp, 3, 4> m34Mul(const Matx<_Tp, 3, 4>& A, const Matx<_Tp, 3, 4>& B)
{
	return A*m34ToM44(B);
}

template<typename _Tp>
inline Matx<_Tp, 3, 4> m34Inv(const Matx<_Tp, 3, 4>& A)
{
	return m34FromM44(m34ToM44(A).inv());
}

template<typename _Tp>
inline Matx<_Tp, 3, 4> operator*(const Matx<_Tp, 3, 4>& A, const Matx<_Tp, 3, 4>& B)
{
	return m34Mul(A, B);
}
template<typename _Tp>
inline Point3_<_Tp> operator*(const Matx<_Tp, 3, 4>& A, const Point3_<_Tp>& point)
{
	return Point3_<_Tp>(A * Vec<_Tp, 4>(point.x, point.y, point.z, _Tp(1)));
}
template<typename _Tp>
inline Point3_<_Tp> operator*(const Matx<_Tp, 4, 4>& A, const Point3_<_Tp>& point)
{
	auto v = A * Vec<_Tp, 4>(point.x, point.y, point.z, _Tp(1));
	_Tp s=_Tp(1)/v[3];
	return Point3_<_Tp>(v[0]*s, v[1]*s, v[2]*s);
}
