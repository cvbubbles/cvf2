...
int cvxWaitKey(const std::vector<int>& exitCodes)
{
	int c = 0;
	while (true)
	{
		...
		if (c != 0 && (exitCodes.empty() || std::find(exitCodes.begin(),exitCodes.end(),c) !=exitCodes.end()))
			break;
	}
	return c;
}
